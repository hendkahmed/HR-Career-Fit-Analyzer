...
from questions import QUESTIONS, LIKERT, FUNCTIONS, TRAITS
...
def _header():
    ...
    st.markdown(f"""
      <div class="pv-hero">
        ...
        <span class="pv-badge">Functions Coverage • 10 Tracks</span>
...
def _question_view():
    ...
    choice = st.radio(
        label="",
        options=LIKERT,
        index=LIKERT.index(current) if current in LIKERT else 2,
        horizontal=False
    )
...
def _results_view():
    ...
    st.markdown("""
      <div class="pv-card">
        <div style="font-size:15px;font-weight:700;margin-bottom:10px;">Function Scores</div>
    """, unsafe_allow_html=True)

    func_df = pd.DataFrame({
        "Function": list(result.function_scores.keys()),
        "Score": list(result.function_scores.values())
    ...
    st.markdown("""
      <div class="pv-card">
        <div style="font-size:15px;font-weight:700;margin-bottom:10px;">Work-Style Traits</div>
    """, unsafe_allow_html=True)

    trait_df = pd.DataFrame({
        "Trait": list(result.trait_scores.keys()),
        "Score": list(result.trait_scores.values())
    }).sort_values("Score", ascending=True)
    ...
    with c2:
        st.button("Back to Review Answers", use_container_width=True, on_click=_go_review)

    st.markdown("</div>", unsafe_allow_html=True)

def _go_review():
    st.session_state.idx = max(0, len(QUESTIONS) - 1)
